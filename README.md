...
- **🔐 인증 시스템**: OpenAI API 키 입력 또는 관리자 로그인
...
#### 🔐 관리자 모드
환경변수에 OpenAI API 키와 관리자 계정을 설정하면, 관리자는 별도의 API 키 입력 없이 로그인할 수 있습니다:

```env
# 시스템 OpenAI API 키 (관리자용)
OPENAI_API_KEY=sk-proj-your-openai-api-key
...
#### 🤖 사용자 모드
일반 사용자는 개인 OpenAI API 키를 입력하여 AI 자동 매핑 기능을 사용할 수 있습니다.
...
3. 환경 변수 설정:
   ...
   - `OPENAI_API_KEY=your-openai-key` (관리자용 - 선택사항)
   - `ADMIN_USERNAME=admin` (관리자용 - 선택사항)
   - `ADMIN_PASSWORD=your-password` (관리자용 - 선택사항)
   - Gmail 설정 등
4. 배포 완료
